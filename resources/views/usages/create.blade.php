<x-app-layout>
    ...
    <div class="form-page">
        <div class="form-page__head">
            <div class="eyebrow eyebrow--accent">Usages IA · Déclaration</div>
            <h1>Déclarer un <em>usage.</em></h1>
            <p class="lead">
                Renseignez le nom, le type et le domaine de cet outil. Le questionnaire AI Act
                viendra ensuite préciser les caractéristiques nécessaires à la classification.
            </p>
        </div>

        <form method="POST" action="{{ route('usages.store') }}" class="form-card">
            @csrf
            @include('usages._form')

            <div class="form-actions">
                <a href="{{ route('usages.index') }}" class="btn-link">← Annuler</a>
