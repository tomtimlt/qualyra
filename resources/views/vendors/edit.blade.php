<x-app-layout>
    <x-slot name="header">
        <a href="{{ route('vendors.index') }}" style="color: inherit; text-decoration: none">Qualyra / Fournisseurs IA</a> / <b>{{ $vendor->name }}</b> / Modifier
    </x-slot>

    <div class="form-page">
        <div class="form-page__head">
            <div class="eyebrow eyebrow--accent">Fournisseurs IA · Édition</div>
            <h1>Modifier <em>{{ $vendor->name }}.</em></h1>
        </div>

        <form method="POST" action="{{ route('vendors.update', $vendor) }}" class="form-card">
            @csrf
            @method('PATCH')
